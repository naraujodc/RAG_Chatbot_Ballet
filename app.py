...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Classical Ballet RAG Assistant",
    page_icon="🩰",
    layout="wide"
)
...
# Temporary storage for Example Questions button clicks
...
with st.sidebar:
    ...
    - YouTube video transcripts, including documentaries
    - Scholarly literature about the history and economics of ballet
    - Webpages describing types of ballets, famous pieces, etc.
...
input_prompt = st.chat_input("Ask a question about classical ballet...")
prompt_to_process = None

# Check if an example question has been selected
if st.session_state.temp_prompt:
    prompt_to_process = st.session_state.temp_prompt
    st.session_state.temp_prompt = None

elif input_prompt:
    prompt_to_process = input_prompt
    ...
    with st.chat_message("assistant"):
        with st.spinner("Searching database and generating answer..."):
            try:
                ...
                # Get answer
                result = agent.ask(prompt_to_process)
...
with st.expander("💡 Example Questions"):
    examples = [
        ...
        "Who are some famous composers every dancer should know?",
        "What are the main characteristics of romantic ballets?",
        "How does the Balanchine method differ from the RAD method?"
    ]
